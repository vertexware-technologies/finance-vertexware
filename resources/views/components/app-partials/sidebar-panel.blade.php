                                        ...
                                        @foreach ($menu['submenu'] as $keyMenu => $submenu)
                                            <li
                                                ...
                                                <a href="{{ route($submenu['route_name']) }}"
                                                    class="{{ $submenu['route_name'] === $pageName ? 'text-primary dark:text-accent-light font-medium' : 'text-slate-600 hover:text-slate-800 dark:text-navy-200 dark:hover:text-navy-50' }} flex items-center justify-between p-2 text-xs+ tracking-wide outline-none transition-[color,padding-left] duration-300 ease-in-out hover:pl-4">
                            ...
                            @else
                                <li @if ($menu['route_name'] === $pageName) x-init="$el.scrollIntoView({block:'center'});" @endif>
                                    <a href="{{ route($menu['route_name']) }}"
                                        class="{{ $menu['route_name'] === $pageName ? 'text-primary dark:text-accent-light font-medium' : 'text-slate-600  hover:text-slate-800 dark:text-navy-200 dark:hover:text-navy-50' }} flex py-2 text-base tracking-wide outline-none transition-colors duration-300 ease-in-out">
                                        @if (isset($menu['icon']))
                                            <i class="{{ $menu['icon'] }} fa-sm mx-2 my-auto w-5"></i>
                                        @endif {{ $menu['title'] }}
                                    </a>
                                </li>
                            @endif
